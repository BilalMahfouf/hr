...
public sealed class PunchAndRecordQueryHandlerTests : AttendenceTestBase
{
    ...
    private static async Task<AttendenceMachine> SeedMachineAsync(
        ...
        string ipAddress)
    {
        var machine = AttendenceMachine.Create(MachineId.New(), ipAddress, 1, MachineType.ZKTecoSdk, 8080);
        ...
        DateTime? checkOut = null)
    {
        ...
        if (checkOut is not null)
        {
            record.RegisterCheckOut(
                checkOut.Value,
                new Modules.Attendence.Domain.AttendenceRecords.WorkSchedule(TimeSpan.FromHours(8), checkOut.Value));
        }
