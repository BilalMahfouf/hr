...
public static class CreateCheckout
{
    ...
    ) : ICommandHandler<CreateSubscriptionCheckoutCommand, Response>
    {
        ...
        )
        {
            ...
            {
                return Result<Response>.Failure(SubscriptionErrors.ActiveSubscriptionAlreadyExist);
            }

            var existingPayment = await db
                .SubscriptionPayments.Select(e => new
                {
                    e.Id,
                    e.ProviderPaymentId,
                    e.IdempotencyKey,
            ...
            {
                Response<CheckoutResponse>? existingCheckout = await chargilyPayClient.GetCheckout(
                    existingPayment.ProviderPaymentId
                );
                if (existingCheckout is null || existingCheckout.Value.CheckoutUrl is null)
                {
                    return Result<Response>.Failure(
                        SubscriptionErrors.FailedToRetrieveCheckout(existingPayment.Id)
                    );
                }
            ...
            {
                return Result<Response>.Failure(
                    SubscriptionPlanErrors.SubscriptionPlanNotFound(command.PlanId)
                );
            }
            ...
            if (subscription.Status == SubscriptionStatus.Trialing)
            {
                await db.SaveChangesAsync(cancellationToken);
                return Result<Response>.Success(
                    new Response(null, subscription.Status.ToString(), subscription.Id)
                );
            }
            ...
            {
                return Result<Response>.Failure(
                    SubscriptionErrors.FailedToRetrieveCheckout(Guid.Empty)
                );
            }
    ...
    public sealed class Endpoint : IEndpoint
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            app.MapPost(
                ...
                .WithDescription(
                    "Creates a new subscription checkout for the current authenticated doctor and returns the checkout URL or trialing status."
                )
